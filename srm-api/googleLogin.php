...
if(isset($_REQUEST['gmail'])){
    $gmail = addslashes($_REQUEST['gmail']);

$query = "SELECT * FROM user_login_new WHERE email = '$gmail'";

$run_it = mysqli_query($conn1,$query);


$num = mysqli_num_rows($run_it);
if($num=='0'){
    
    	echo "{\n\"error\":false,";
	echo "\"error_msg\":\"Please Use Registration Number and Password for last time\",";
	echo "\"code\":\"404\"}";
...
while($ud = mysqli_fetch_assoc($run_it)){
    $data_from_hb = $ud;
    
}

$regno = $data_from_hb["regno"];
$pass = $data_from_hb["pass"];


if (isset($regno) && isset($pass))
{
...
// 	$pa = addslashes($_REQUEST['pass']);
	...
	$cc = new cURL();
	if ($data = $cc->get('evarsity.srmuniv.ac.in/srmswi/usermanager/youLogin.jsp?txtSN=' . $regno . '&txtPD=' . $pass . '&txtPA=1'))
	{
...
// 			$run = mysqli_query($conn1, $stmt);
    	...
    	echo "{\n\"error\":true,";
			echo "\"error_msg\":\"YOUR PASSWORD SEEMS TO HAVE CHANGED FROM LAST LOGIN PLEASE RELOGIN\",";
			echo "\"code\":\"404\"}";
